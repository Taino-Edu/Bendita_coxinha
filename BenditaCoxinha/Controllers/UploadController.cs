// =============================================================================
// UploadController.cs â€” Upload de imagens para a loja
// POST /api/upload/image  â†’ salva em wwwroot/uploads/ e retorna URL pÃºblica
...
public class UploadController : ControllerBase
{
    ...
    /// <summary>
    /// Faz upload de uma imagem (JPEG, PNG ou WebP, mÃ¡x. 5 MB).
    /// Retorna a URL pÃºblica do arquivo salvo em /uploads/.
    /// </summary>
    [HttpPost("image")]
    [Authorize(Policy = "AdminOnly")]
    [RequestSizeLimit(6 * 1024 * 1024)] // margem acima do limite de negÃ³cio
    ...
    private async Task<IActionResult> ProcessImageUpload(IFormFile? file, string relativeDir)
    {
        ...
        if (file.Length > MaxFileSizeBytes)
            return BadRequest(new ErrorResponse($"O arquivo excede o tamanho mÃ¡ximo de 5 MB (recebido: {file.Length / 1024} KB)."));

        if (!AllowedMimeTypes.Contains(file.ContentType))
            return BadRequest(new ErrorResponse($"Tipo de arquivo nÃ£o permitido: {file.ContentType}. Use JPEG, PNG ou WebP."));

        var ext = Path.GetExtension(file.FileName).ToLowerInvariant();
        if (!AllowedExtensions.Contains(ext))
            return BadRequest(new ErrorResponse($"ExtensÃ£o nÃ£o permitida: {ext}. Use .jpg, .jpeg, .png ou .webp."));

        // Valida magic bytes â€” impede upload de arquivos renomeados com extensÃ£o falsa
        if (!HasValidImageMagicBytes(file))
            return BadRequest(new ErrorResponse("O arquivo nÃ£o Ã© uma imagem vÃ¡lida."));
...
/// <summary>Resposta de erro de validaÃ§Ã£o.</summary>
