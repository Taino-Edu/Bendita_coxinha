...
namespace BenditaCoxinha.Middleware;

/// <summary>
/// Verifica se um Operator tem a permissÃ£o necessÃ¡ria para acessar uma rota.
/// Admin sempre passa. Customer Ã© bloqueado nas rotas admin.
/// </summary>
public class OperatorPermissionMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        // SÃ³ verificar rotas /api/ (exceto auth e health)
        ...
        if (role == UserRole.Operator)
        {
            ...
            if (string.IsNullOrEmpty(permissionsClaim))
            {
                ...
                await context.Response.WriteAsJsonAsync(new { Message = "Operador sem permissÃµes configuradas." });
                return;
            }
            ...
            // Sempre permite rotas genÃ©ricas de leitura prÃ³pria
            if (path.StartsWith("/api/user/me") || path.StartsWith("/api/comanda/mesa"))
                hasAccess = true;

            if (!hasAccess)
            {
                _logger.LogWarning("Operator {UserId} sem permissÃ£o para {Path}", user.FindFirst("sub")?.Value, path);
                context.Response.StatusCode = StatusCodes.Status403Forbidden;
                await context.Response.WriteAsJsonAsync(new { Message = "Sem permissÃ£o para esta aÃ§Ã£o." });
                return;
            }
        }
