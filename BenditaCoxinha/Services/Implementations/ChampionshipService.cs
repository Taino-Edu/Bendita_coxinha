// =============================================================================
// ChampionshipService.cs â€” ImplementaÃ§Ã£o de Campeonatos
// =============================================================================
...
public class ChampionshipService : IChampionshipService
{
    ...
    public async Task DeleteAsync(Guid id)
    {
        var ch = await _db.Championships.Include(c => c.Participants).FirstOrDefaultAsync(c => c.Id == id)
            ?? throw new InvalidOperationException("Campeonato nÃ£o encontrado.");

        if (ch.Status != ChampionshipStatus.Finalizado && ch.Status != ChampionshipStatus.Cancelado)
            throw new InvalidOperationException("SÃ³ Ã© possÃ­vel excluir campeonatos Finalizados ou Cancelados.");
    ...
    {
        var ch = await _db.Championships.FindAsync(id) ?? throw new InvalidOperationException("Campeonato nÃ£o encontrado.");
        ch.Status = newStatus;
        await _db.SaveChangesAsync();
        return ch;
    }
    ...
    {
        // Verifica se o usuÃ¡rio jÃ¡ estÃ¡ inscrito para evitar duplicata
        var jaInscrito = await _db.ChampionshipParticipants
            .AnyAsync(p => p.ChampionshipId == championshipId && p.UserId == userId);
        if (jaInscrito)
            throw new InvalidOperationException("UsuÃ¡rio jÃ¡ estÃ¡ inscrito neste campeonato.");

        // Gera nÃºmero sequencial de jogador (Ãºltimo + 1)
    ...
    {
        var p = await _db.ChampionshipParticipants.FindAsync(participantId)
            ?? throw new InvalidOperationException("Participante nÃ£o encontrado.");
