...
public class AnnouncementService : IAnnouncementService
{
    ...
    public async Task<AnnouncementDto> CreateAsync(CreateAnnouncementRequest request, Guid adminId)
    {
        ...
        _logger.LogInformation("An̼ncio '{Title}' criado por admin {AdminId}", ann.Title, adminId);
        return Map(ann);
    }
    ...
    public async Task<AnnouncementDto> UpdateAsync(Guid id, UpdateAnnouncementRequest request)
    {
        var ann = await _db.Announcements.FindAsync(id)
            ?? throw new InvalidOperationException("An̼ncio ṇo encontrado.");
        ...
        ann.ExpiresAt = request.ExpiresAt; // null = sem expira̤̣o; sempre atualiza para permitir limpeza
