// =============================================================================
// ComandaDtos.cs â€” DTOs de Comanda e Itens
// =============================================================================
...
// -------------------------------------------------------------------------
// Request: Adicionar item Ã  comanda (usado pelo Hub e pelo Controller REST)
// -------------------------------------------------------------------------

/// <summary>Dados necessÃ¡rios para adicionar um item a uma comanda.</summary>
public class AddItemToComandaRequest
{
    /// <summary>ID do produto no estoque (nullable se for carta TCG).</summary>
    public Guid? ProductId { get; set; }

    /// <summary>ID da carta no cache MongoDB (nullable se for produto fÃ­sico).</summary>
    public string? CardCacheId { get; set; }

    /// <summary>Nome do item (preenchido automaticamente pelo serviÃ§o).</summary>
    [MaxLength(200)]
    public string ItemName { get; set; } = string.Empty;

    /// <summary>PreÃ§o unitÃ¡rio em centavos (preenchido pelo serviÃ§o a partir do estoque).</summary>
...
/// <summary>Snapshot da comanda para exibiÃ§Ã£o no dashboard e confirmaÃ§Ãµes.</summary>
public class ComandaDto
{
    ...
    /// <summary>Saldo de pontos do cliente â€” exibido na modal de fechamento.</summary>
    public int  UserPointsBalance  { get; set; }
    /// <summary>Saldo de cashback do cliente em centavos â€” exibido na modal de fechamento.</summary>
...
public class CloseComandaRequest
{
    ...
    /// <summary>ObservaÃ§Ã£o opcional (ex: "troco de R$50,00").</summary>
    public string? Observacao { get; set; }

    /// <summary>Segundo mÃ©todo de pagamento para split (Cashback, Pontos, Dinheiro, Pix, etc.).</summary>
    public string? SecondPaymentMethod { get; set; }

    /// <summary>Valor pago pelo segundo mÃ©todo em centavos. Zero = sem split.</summary>
    public int SecondPaymentAmountInCents { get; set; } = 0;

    /// <summary>
    /// Quando PaymentMethod == Crediario, este campo indica se a dÃ­vida deve ser
    /// adicionada a um crediÃ¡rio aberto existente (fornece o Id do crediÃ¡rio)
    /// ou se deve criar uma nova conta (null = nova conta com prazo prÃ³prio de 30 dias).
    /// </summary>
...
// -------------------------------------------------------------------------
// EdiÃ§Ã£o de comanda fechada (Admin only)
// -------------------------------------------------------------------------

/// <summary>Request para editar uma comanda jÃ¡ fechada.</summary>
...
/// <summary>Wrapper genÃ©rico para resultados paginados da API.</summary>
...
// Adapte os campos conforme a documentaÃ§Ã£o real da API
// -------------------------------------------------------------------------
...
/// <summary>Set/ExpansÃ£o de um jogo TCG.</summary>
